  ...
  try {
   ...
   //var old_date = '2023-11-04';
    ...
    request(options2, function (error, response2) {
      ...
      //console.log(result2.dataset.data);
           ...
           if(result.results.length != 0 && result.results.length !== undefined && result.results.length != null)
             {
                 ...
                 console.log("No data Found on hubspot for "+id) 
             }
...
const delete_promise = (email) => new Promise(async(resolve, reject)=> {
    try{

    	      var options = { 
    	      method: 'DELETE', 
			  url: 'https://api.hubapi.com/contacts/v1/contact/email/'+email,
			  headers: 
			  {
   ...
   const check_delete = async(email) =>{
   try {
    const data = await delete_promise(email);
    return data;
   } catch (error) {
    return error
...
const test_delete = async(email)=>{
   const exist_id = await check_delete(email);
   return exist_id;
